...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      // Use gemini-1.5-flash which is the current stable vision model
...
REMEMBER: Extract ANY identifying information you see. Even "Red brick building with '123' on door" is better than "apartment building".`
    ...
    if (!latitude || !longitude) {
      ...
      if (!hasApiKey) {
        ...
        // DEBUG: Show why it failed in the subtitle
        const errorReason = !visionResult ? 'Vision ID failed' : 'Places lookup failed'
        subtitle = `Debug: ${errorReason}. Location: ${latitude.toFixed(4)}, ${longitude.toFixed(4)}`
      }
    }
